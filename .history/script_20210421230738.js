...
let preçoTotal = 0;
const vemQueEaSoma = async (price) => {
  preçoTotal += price;
  document.querySelector('.total-price').innerText = preçoTotal;
  console.log(preçoTotal);
};

function cartItemClickListener(event) {
  event.target.remove();
  const targetText = event.target.innerText.split('$');
  vemQueEaSoma(targetText[1] *= -1);
  updateLocalStorage();
}
...
function esvaziarCarrinho() {
  const botaoVazio = document.querySelector('.empty-cart');
    botaoVazio.addEventListener('click', () => {
